...
public class GpuMergedPointCloudBootstrap : MonoBehaviour
{
    ...
    /// <summary>Suppresses the whole bootstrap for this scene load.
    ///
    /// InterventionSessionBootstrap runs at DefaultExecutionOrder(-10000) and disables the
    /// point cloud components in RGB-panel mode — but this bootstrap runs at -500, i.e.
    /// AFTER, and Unity calls Awake() even on a component whose `enabled` is false. So it
    /// re-created an ENABLED merged loader that immediately opened a 3-topic /pc
    /// subscriber the earlier disable pass had already walked past. That stray subscriber
    /// inflated the session's ZMQ peer count and kept Python in full point-cloud mode.
    /// A static flag is the only thing that reliably crosses that ordering gap.</summary>
    public static bool SuppressBootstrap;
    ...
    private void Awake()
    {
        ...
        {
            Debug.Log("[GpuMergedPointCloudBootstrap] Suppressed (RGB panel mode): not creating "
                    + "the merged point cloud loader, so no /pc subscriber is opened.");
            return;
        }
        if (!enableBootstrap)
            return;
    ...
    private void EnsureCloudObject(string objectName, string topic)
    {
        ...
        pointCloud.parentToSimScene = parentToSimScene;
    ...
    private void EnsureMergedCloudObject()
    {
        ...
        mergedLoader.expectedSessionIndex = expectedSessionIndex;
        ...
        mergedLoader.parentToSimScene = parentToSimScene;
        mergedLoader.waitForSceneAnchorBeforeFirstDraw = waitForSceneAnchorBeforeFirstDraw;
    ...
    /// <summary>Set the render mode and mark it as an explicit choice.</summary>
    /// <remarks>
    /// Without this, a caller that assigns <see cref="renderMode"/> directly is silently
    /// ignored: <see cref="GetEffectiveRenderMode"/> falls through to the legacy
    /// <c>useDepthTestDebugMode</c> branch because <c>_hasExplicitRenderModeSelection</c> is
    /// a private serialized field nothing sets at runtime. That is exactly what happened to
    /// InterventionSessionBootstrap's OverlayDepthWrite request — the loader ended up in
    /// OverlayTransparent (ZWrite=0), so the ATW compositor had no depth to reproject the
    /// cloud with and it swam during head movement.
    /// </remarks>
    public void SetRenderModeExplicit(PointCloudRenderMode mode)
    {
        renderMode = mode;
