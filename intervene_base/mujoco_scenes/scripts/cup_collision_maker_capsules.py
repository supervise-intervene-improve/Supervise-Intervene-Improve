...
def make_bottom_geom(
    ...
    return (
        ...
        f'class="cup_collision"/>'
    )


def make_wall_capsule_geoms(
    name: str,
    center_raw: np.ndarray,
    R_raw_from_local: np.ndarray,
    local_xy_center: np.ndarray,
    ...
    capsule_radius = 0.5 * wall_thickness
    capsule_half_length = max(1e-6, 0.5 * wall_height - capsule_radius)

    dtheta = 2.0 * math.pi / num_capsules
    ...
    for i in range(num_capsules):
        ...
        # vertical capsule, axis along local Z
        R_local_geom = np.eye(3)
        ...
        geoms.append(
            f'      <geom name="{name}_wallcap_{i:02d}" type="capsule" '
            f'pos="{fmt(pos_body)}" quat="{fmt(quat_body)}" '
            f'size="{fmt([capsule_radius, capsule_half_length])}" '
            f'class="cup_collision"/>'
        )
...
def make_top_rim_capsule_geoms(
    name: str,
    center_raw: np.ndarray,
    R_raw_from_local: np.ndarray,
    local_xy_center: np.ndarray,
    ...
    top_center_z = z_min + height - rim_margin + z_offset
    dtheta = 2.0 * math.pi / num_capsules

    # arc length covered by one segment
    segment_len = cap_tangential_scale * (2.0 * math.pi * cap_mid_radius / num_capsules)
    capsule_radius = 0.5 * cap_radial_thickness
    capsule_half_length = max(1e-6, 0.5 * segment_len - capsule_radius)
    ...
    for i in range(num_capsules):
        ...
        # capsule axis is local Z by default, so rotate local Z -> tangent
        R_local_geom = np.column_stack([radial, vertical, tangent])
        ...
        geoms.append(
            f'      <geom name="{name}_toprim_{i:02d}" type="capsule" '
            f'pos="{fmt(pos_body)}" quat="{fmt(quat_body)}" '
            f'size="{fmt([capsule_radius, capsule_half_length])}" '
            f'class="cup_collision"/>'
        )
...
def generate_cup_xml(cfg: CupShellConfig) -> str:
    ...
    wall_geoms, inner_radius = make_wall_capsule_geoms(
        name=cfg.name,
        center_raw=center_raw,
        R_raw_from_local=R_raw_from_local,
        local_xy_center=local_xy_center,
        ...
        num_capsules=cfg.num_wall_capsules,
        show=cfg.show_collision,
        frame_pos_offset_local=frame_pos_offset_local,
        frame_euler_deg_local=frame_euler_deg_local,
    )
    ...
    if cfg.top_cap_enabled:
        top_rim_capsules = make_top_rim_capsule_geoms(
            name=cfg.name,
            center_raw=center_raw,
            R_raw_from_local=R_raw_from_local,
            local_xy_center=local_xy_center,
        ...
        body_lines.extend(top_rim_capsules)
...
"""
    ...
    if cfg.print_debug:
        ...
        print(f"  wall_capsules               = {cfg.num_wall_capsules}")
...
if __name__ == "__main__":
    common_kwargs = dict(
        mesh_file="/path/to/Intervention_IL_AR/intervene_base/franka_emika_panda/cups/cup_nontextured.stl",

        num_wall_capsules=26,
        wall_thickness=0.003,
        wall_outward_offset=0.003,
        bottom_thickness=0.004,
        rim_margin=0.002,

        # usually smoother if off
        top_cap_enabled=False,
        top_cap_radial_thickness=0.002,
        top_cap_tangential_scale=1.0,
        top_cap_outward_offset=0.0,
        top_cap_z_offset=0.0,

        bottom_region_fraction=0.15,
        bottom_radius_quantile=0.95,
        radius_padding=0.001,

        auto_detect_axis=False,
        axis_if_fixed="z",
    ...
    cups = [
        ...
        CupShellConfig(
            name="cup2",
            body_pos=(0.45, -0.12, 0.24),
            mesh_name="cup_mesh_2",
            material_name="cup_green",
            mesh_scale=(1.20, 1.20, 1.00),
    ...
    xml_name = f"generated_{len(cups)}_cups_CAPSULES"
