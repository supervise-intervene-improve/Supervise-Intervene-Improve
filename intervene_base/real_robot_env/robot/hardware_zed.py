...
from zmq_server import send, start_sender_process
import struct
import argparse

def get_intrinsics(CAMERA):
    intrinsics = (
        CAMERA.get_camera_information().camera_configuration.calibration_parameters
    )
...
def stream_data(CAMERA,rgb, d, n):
    ...
    _getrinsics = get_intrinsics(CAMERA)
...
if __name__ == "__main__":
    # ------------------------
    # 1️⃣ Process command-line arguments
    # ------------------------
    parser = argparse.ArgumentParser(description="ZED camera test")
    parser.add_argument("--serial", type=int, help="Serial number of the ZED camera to connect", required=True,)
    parser.add_argument("--name", type=str, help="name of the camera", required=True,)
    ...
    print("Connected ZED cameras:")
    for i, device in enumerate(devices):
        print(f"{i}: {device.serial_number}")

    import pyzed.sl as sl

    # ------------------------
    # 4️⃣ Initialize ZED camera
    # ------------------------
    ...
    if args.serial:
        ...
        print(f"Using camera with serial number: {args.serial}")
    else:
        init.set_from_serial_number(devices[0].serial_number)
        print(f"No serial provided. Using first detected camera with serial number: {devices[0].serial_number}")

    zed = sl.Camera()
    status = zed.open(init)
    print("ZED open result:", status)
    ...
    while True:
        ...
        if zed.grab(runtime) == sl.ERROR_CODE.SUCCESS:
            ...
            image_left_np = image_left_np[:, :, :3]  # BGRA to BGR
            # print("Image shape:", image_left_np.shape)
            depth_np = depth.get_data()
            # end_total = time.time()
            # print(f"[Total] Frame verarbeitet in {end_total - start_total:.4f} Sekunden")


            stream_data(zed, image_left_np, depth_np, args.name)
            frame_count += 1
        end = time.time()
