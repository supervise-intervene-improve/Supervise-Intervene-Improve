...
if [[ ! -d "${DATA_ROOT}" ]]; then
  ...
  echo "[HINT] Run ./utils/build_cups_abs_shift1_wrist_rerender_gripper_fixed_dataset.sh first." >&2
  exit 1
fi
...
OUTDIR=./outputs/cups_act_abs_shift1_wrist_rerender_gripper_fixed_${RUN_ID}
MODEL_REPO_ID=anonymous/cups_act_abs_shift1_wrist_rerender_gripper_fixed_${RUN_ID}
...
"${LEROBOT_TRAIN}" \
  ...
  --output_dir=${OUTDIR} \
  --job_name=cups_act_abs_shift1_wrist_rerender_gripper_fixed_${RUN_ID} \
