...
class XRNodeEntry:
    ...
    first_heartbeat: float = field(default_factory=lambda: 0.0)
    last_heartbeat: float = field(default_factory=lambda: 0.0)
    heartbeat_count: int = 0

    def touch(self) -> None:
        now = time.time()
        if self.first_heartbeat <= 0.0:
            self.first_heartbeat = now
    ...
    def age_seconds(self, now: Optional[float] = None) -> float:
        ...
        return max(0.0, current_time - self.first_heartbeat)
